...
SCHEME_CLEANUPCACHE=$1  # CleanupSpec (final version)
SCHEME_INVISISPEC=$2
BENCHMARK=$3                # Benchmark name, e.g. perlbench
RUN_CONFIG=$4                      # Name of configuration being run (will decide output directory name)
IS_DEBUG=$5
DEBUG_FLAG=$6
IS_SPEC=true
...
if [[ "$BENCHMARK" == "spectre" ]]; then
    BENCHMARK_CODE="spectre"
	IS_SPEC=false
fi
# Sanity check
...
OUTPUT_DIR=$GEM5_PATH/results/$CHECKPOINT_CONFIG/$RUN_CONFIG/${SCHEME_CLEANUPCACHE}/$BENCHMARK
echo "checkpoint directory: " $CKPT_OUT_DIR
else 
RUN_DIR="../attack_code"
OUTPUT_DIR=$GEM5_PATH/results/$RUN_CONFIG/${SCHEME_CLEANUPCACHE}/$BENCHMARK
fi
# Output Dir
...
cmd="--debug-flags=$DEBUG_FLAG --debug-start=120000000000  \
    --outdir=$OUTPUT_DIR $GEM5_PATH/configs/example/attack_code_config.py \
    --benchmark=$BENCHMARK --benchmark_stdout=$OUTPUT_DIR/$BENCHMARK.out \
    --benchmark_stderr=$OUTPUT_DIR/$BENCHMARK.err \
    --num-cpus=1 --mem-size=4GB \
    --l1d_assoc=8 --l2_assoc=16 --l1i_assoc=4 \
    --cpu-type=DerivO3CPU --needsTSO=0 --scheme_invisispec=$SCHEME_INVISISPEC \
    --scheme_cleanupcache=$SCHEME_CLEANUPCACHE \
    --num-dirs=1 --ruby   \
    --network=simple --topology=Mesh_XY --mesh-rows=1 --prog-interval=0.003MHz "


if [[ $IS_DEBUG == "needDebug" ]];then
   cmd="gdb --args $GEM5_PATH/build/X86_MESI_Two_Level/gem5.debug "$cmd
else
        cmd="$GEM5_PATH/build/X86_MESI_Two_Level/gem5.opt "$cmd
fi
echo $cmd
#$cmd